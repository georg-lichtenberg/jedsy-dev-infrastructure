...
# Das Update mit dem richtigen Endpunkt durchführen
echo "Aktualisiere device_type auf: $DEVICE_TYPE"

# Formular-Parameter für die POST-Anfrage
FORM_DATA="device_type=${DEVICE_TYPE}"

# Debug: Zeige die vollständige curl-Anfrage
echo "Sende POST-Anfrage an: ${API_BASE_URL}/admin/device/${DEVICE_ID}"
echo "Formular-Daten: $FORM_DATA"

# Hole Session-Cookie von der Login-Seite
echo "Hole Session-Cookie..."
COOKIE_JAR=$(mktemp)
curl -s -c "$COOKIE_JAR" "${API_BASE_URL}/login" > /dev/null

# Sende die POST-Anfrage mit dem Session-Cookie
UPDATE_RESPONSE=$(curl -s -X POST \
    -H "Authorization: Bearer $ACCESS_TOKEN" \
    -H "Content-Type: application/x-www-form-urlencoded" \
    -b "$COOKIE_JAR" \
    -d "$FORM_DATA" \
    "${API_BASE_URL}/admin/device/${DEVICE_ID}")

# Lösche temporäre Cookie-Datei
rm -f "$COOKIE_JAR"

echo "Update durchgeführt. Überprüfe Ergebnis..."

# Hier müssten wir den Edit-Formular-Endpunkt simulieren
FORM_DATA="device_type=${DEVICE_TYPE}"
UPDATE_RESPONSE=$(curl -s -X POST \
    -H "Authorization: Bearer $ACCESS_TOKEN" \
    -H "Content-Type: application/x-www-form-urlencoded" \
...
# Geräteinformationen erneut abfragen
DEVICE_RESPONSE=$(curl -s -H "Authorization: Bearer $ACCESS_TOKEN" "${API_BASE_URL}/admin/device/by-name/${DEVICE_NAME}")

# Extrahiere den aktuellen Device Type
CURRENT_TYPE=$(echo "$DEVICE_RESPONSE" | grep -o 'Type:</strong> [^<]*' | sed 's/Type:<\/strong> \(.*\)/\1/')

echo "Aktueller device_type: $CURRENT_TYPE"

if [ "$CURRENT_TYPE" = "$DEVICE_TYPE" ]; then
    echo "✅ Update erfolgreich!"
    exit 0
else
    echo "❌ Update fehlgeschlagen! Aktueller Wert: '$CURRENT_TYPE', Gewünschter Wert: '$DEVICE_TYPE'"
    # Gib die Antwort aus, um zu sehen, was schief gelaufen ist
    echo -e "\nDebug: Server-Antwort von Update-Anfrage:"
    echo "$UPDATE_RESPONSE" | head -n 30
    exit 1
fi
